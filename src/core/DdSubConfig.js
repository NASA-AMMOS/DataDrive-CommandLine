...
class DdSubConfig extends DdConfig {
    ...
    processOptions(program) {
        ...
        this.fileRegexObj = this.fileRegex
            ? new RegExp(this.fileRegex)
            : undefined;
    ...
    async validateConfigs() {
        if (this.disableDownload && this.pluginPath === undefined) {
            DdUtils.errorAndExit(
                "Disable download option enabled but missing plugin path option.",
            );
        }
        ...
        if (this.packageName) {
            if (this.fileRegex !== undefined && this.fileWildcard !== "*") {
                DdUtils.errorAndExit(
                    "Please specify only 1 filter option; regex or wildcard.",
                );
            }
            if (this.savedSearchName !== undefined) {
                DdUtils.errorAndExit(
                    "Cannot specify a saved search and package name together.",
                );
            }
            await this.validateFolderStateFile(
                this.savedPackageNamePath,
                this.savedSavedSearchNamePath,
                this.packageName,
            );
            return true;
        }
        ...
        DdUtils.errorAndExit(
            "Please specify a package name or a saved search name.",
        );
    }
     ...
     * @param {String} name
     */
    async validateFolderStateFile(
        filenameShouldExist,
        filenameShouldNotExist,
        name,
    ) {
        ...
        if (fs.existsSync(filenameShouldNotExist)) {
            DdUtils.errorAndExit(
                `There is a ${filenameShouldNotExist} file in this folder. Exiting.`,
            );
        }
        ...
        if (fs.existsSync(filenameShouldExist)) {
            ...
            if (name !== file.trim()) {
                DdUtils.errorAndExit(
                    `Package name or saved search name specified (${name}) is not the same as the one specified in ${filenameShouldExist}.`,
                );
            }
        }
    }
    ...
    getOutputLocation(filepath, filename) {
        ...
        if (DdUtils.isEmptyOrNullString(_outputDir)) return null;
    ...
    satisfiesFilter(fullpath) {
        ...
        if (this.fileRegexObj) {
            // always do user given regex first and then wildcard
